 ...
 * In the future we probably will go on the path of customizable recorders, to be able to debug these customized apps, we need to store 1) which component is initialized and/or rendered, and in which order; 2) potential Error stacktraces; 3) relevant browser/client information (user_agent, flash version etc).
...
/**
 * Transport to send logs using JSONP to server.
 * 
 * @method  JSONPTransport
 * @param {string} config.api_endpoint url to publish the logs to
 */
function JSONPTransport(config) {
...
JSONPTransport.prototype = {
  ...
  /**
   * Add the logs to the store and send them if 1) the string is too lang for JSONP
   * or 2) the message is an error message.
   * 
   * @method  transport
   ...
   *
   * @method  appendMessageToQuerystring
   * @param  {object}  log
   */
   ...
   *
   * @method send
   * @param {function} success  optional success hanlder
